  ...
  function getItems(count: number) {
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        ...
        // Pour la traînée
        trailLength: 50 + Math.random() * 60, // px
        // Couleurs arc-en-ciel pour la traînée
        trailHue: Math.floor(Math.random() * 360),
        trailSaturation: 80 + Math.random() * 20, // %
        trailLight: 55 + Math.random() * 25, // %
        trailAlpha: 0.18 + Math.random() * 0.18, // opacité variable
      }
    })
  }
    ...
    >
      ...
      <div
        style={{
          ...
          zIndex: -1,
          pointerEvents: "none",
          background: "radial-gradient(circle at 50% 55%, rgba(0,0,0,0) 60%, rgba(0,0,0,0.22) 85%, rgba(0,0,0,0.36) 100%)",
          filter: "blur(2.5px)",
        }}
      />
      {itemsRef.current.map(item => {
        const Logo = logos[item.logoIndex]
        // Animation CSS personnalisée pour chaque objet
        const keyframes = `@keyframes fly-${item.id} {
          0% {
            transform: translate(-50%, -50%) translate(0px, 0px) scale(0.1) perspective(800px) rotateX(30deg);
            opacity: 0.2;
            filter: blur(8px);
          }
          70% {
          ...
          85% {
            transform: translate(-50%, -50%) translate(${item.dx}px, ${item.dy}px) scale(1.2) perspective(100px) rotateX(0deg);
            opacity: 1;
            filter: blur(0px);
          }
          100% {
            transform: translate(-50%, -50%) translate(${item.dx}px, ${item.dy}px) scale(1.3) perspective(60px) rotateX(-10deg);
            opacity: 0;
            filter: blur(0px);
          }
        }`;
        // Traînée : animée sur la même durée mais inverse (de scaleY:1 à scaleY:0, opacity 0.4 à 0)
        const trailKeyframes = `@keyframes trail-${item.id} {
          0% {
            opacity: 0.4;
            transform: scaleY(1);
          }
        ...
        return (
          <div
            ...
            style={{
              ...
              width: item.size,
              height: item.size,
              animation: `fly-${item.id} ${item.duration}s cubic-bezier(0.4,0.1,1,1) ${item.delay}s infinite`, // accentue l'accélération
              opacity: 0.85,
              pointerEvents: "none",
          ...
          >
            ...
            <div
              style={{
                ...
                left: "50%",
                bottom: "50%",
                width: Math.max(2, item.size * 0.23),
                height: item.trailLength,
                background: `linear-gradient(to top, hsla(${item.trailHue},${item.trailSaturation}%,${item.trailLight}%,${item.trailAlpha}), transparent 80%)`,
                borderRadius: 999,
                filter: "blur(2px)",
                transform: `translate(-50%, 0) rotate(${item.angle * 180 / Math.PI}deg)`,
            ...
            <div
              style={{
                ...
                background: "radial-gradient(circle, #fff 0%, #00fff9 60%, transparent 100%)",
                opacity: 0.11,
                borderRadius: "50%",
